...
import { FloorFareDto, PagedResultDtoOfFloorFareDto } from '@shared/models/model-floorfare';
import { GetFloorFareServiceProxy } from '@shared/service-proxies/proxy-floorfare';

@Component({
  templateUrl: './floorfare.component.html',
	animations: [appModuleAnimation()]
})
export class FloorfareComponent extends PagedListingComponentBase<FloorFareDto> {
  active: boolean = false;
  saving: boolean = false;
  code: string;
  group: FloorFareDto[];
  listGrid: Array<FloorFareDto> = [];
  hold: number = 0;
  hold1: number = 1;
  
  constructor(
    injector: Injector,
    private _gbsService: GetFloorFareServiceProxy
  ) {
    super(injector);
  }
  
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    this._gbsService.getfloorfare(request.skipCount, request.maxResultCount)
      .finally(() => {
        finishedCallback();
      })
      .subscribe((result: PagedResultDtoOfFloorFareDto) => {
  ...
  protected delete(entity: FloorFareDto): void {
