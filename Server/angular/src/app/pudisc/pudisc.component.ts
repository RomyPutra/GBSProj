...
import { PUDiscDto, PagedResultDtoOfPUDiscDto } from '@shared/models/model-pudisc';
import { GetPUDiscServiceProxy } from '@shared/service-proxies/proxy-pudisc';

@Component({
  templateUrl: './pudisc.component.html',
	animations: [appModuleAnimation()]
})
export class PUDiscComponent extends PagedListingComponentBase<PUDiscDto> {
  active: boolean = false;
  saving: boolean = false;
  code: string;
  group: PUDiscDto[];
  listGrid: Array<PUDiscDto> = [];
  hold: number = 0;
  hold1: number = 1;
  
  constructor(
    injector: Injector,
    private _gbsService: GetPUDiscServiceProxy
  ) {
    super(injector);
  }
  
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    this._gbsService.getPUDisc(request.skipCount, request.maxResultCount)
      .finally(() => {
        finishedCallback();
      })
      .subscribe((result: PagedResultDtoOfPUDiscDto) => {
  ...
  protected delete(entity: PUDiscDto): void {
