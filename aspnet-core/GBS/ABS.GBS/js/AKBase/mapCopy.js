...
if (SKYSALES.Class.UnitMapContainer === undefined) {
    SKYSALES.Class.UnitMapContainer = function () {
        ...
        //getting the array of the input text initially
        b.initUnitInputArray = function () {
            ...
            for (d = 0; d < this.unitInputArray.length; d += 1) {
                ...
                if (c.isActive === true) {
                    b.activeUnitInput = c //here getting the list of the passengers input text
                   // console.log(b.activeUnitInput);
                }
            }
        ...
        b.updateActiveUnitInput = function (e) {
            var f = 0;
            var h = this.unitInputArray; //put the input here
            var d = null;
            var c = 0;
            var g = null;
            //----------------------------------------If----------------------------------------------------
                ...
                if (e.equipmentIndex === c) {
                    for (f = 0; f < h.length; f += 1) {
                        ...
                        if (d === e) {
                            ...
                            //set the active unit to be in the first textbox on the last itteration
                            if (f == h.length - 1) {
                                b.activeUnitInput = h[0];
                                b.activeUnitInput.input.addClass(this.activeUnitInputClass);
                                d.input.removeClass(this.activeUnitInputClass);
                            }
                        } else {
                            if (g) {
                                g.updateSeatImage("Selected");
...
if (SKYSALES.Class.Unit === undefined) {
    ...
    SKYSALES.Class.Unit.prototype.getImagePath = function (e) {
        ...
        } else if (this.unitGroup == 7 && e != "Reserved") {
            a = c.equipmentImagePath + d + b + this.unitType + b + e + b + this.unitAngle + b + "AB.gif"
        } else if (this.unitGroup == 8 && e != "Reserved") {
