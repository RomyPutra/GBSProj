...
{
    public partial class DetailBreakdowns : System.Web.UI.UserControl
    {
        #region declaration
        //UserSet MyUserSet;
        ...
        protected void panelBreakdown_Callback(object sender, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (cookie != null)
            {
                ...
                for (int i = 0; i < dtOth.Rows.Count; i++)
                {
                    ...
                    else if (dtOth.Rows[i]["FeeCode"].ToString().ToUpper() == "KLIA2" && dtOth.Rows[i]["Origin"].ToString() == model2.TemFlightDeparture.ToString())
                    {
