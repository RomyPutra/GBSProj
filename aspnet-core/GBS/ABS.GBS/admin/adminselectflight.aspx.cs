...
{
    public partial class adminselectflight : System.Web.UI.Page
    {
        #region declaration
        AdminSet AdminSet;
            ...
            if (cookie != null)
            {
                ...
                if (oneway == false)
                {
                    ...
                    if (DataList1.Items.Count <= 0 || DataList2.Items.Count <= 0)
                    {
                        ...
                        Response.Redirect(Shared.MySite.AdminPages.AdminSearchFlight);
                    }
                }
                else
                {
                            ...
                            if (detailCollection.Count > 0)
                            {
                                foreach (BookingTransactionDetail detailData in detailCollection)
                                {
                                    if (CheckValidPNR(detailData.RecordLocator, idxList, listPNR) == true)
                                    {
                                        ...
                                        {
                                            prevSign = absNavitaire.GetBookingByPNR(detailData.RecordLocator, ref errMsg, ref bookingResponse);
                                        }
                                        ...
                                        if (dtPrevJourney.Rows.Count > 1)
                                        {
                                            ...
                                            if (absNavitaire.BookingCommit(detailData.RecordLocator, prevSign, ref errMsg))
                                            {
                                                //assign detail
                                                //detail depart
                                                ...
                                                detailData.LastSyncBy = AdminSet.AdminID;
                                                ...
                                                detailReturn.LastSyncBy = AdminSet.AdminID;
                                ...
                                if (errMsg == "")
                                {
                                    ...
                                    transMain.LastSyncBy = AdminSet.AdminID;
        ...
        protected void FillModelFromDataRow(DataRow[] foundRows, ref ABS.Logic.GroupBooking.Booking.BookingControl.TemFlight model)
        {
            try
            {
                ...
                model.TemFlightPromoCode = foundRows[0]["TemFligtPromoCode"].ToString();
            }
            catch (Exception ex)
            {
                SystemLog.Notifier.Notify(ex);
