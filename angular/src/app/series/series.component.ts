...
import { SeriesDto, PagedResultDtoOfSeriesDto } from '@shared/models/model-series';
import { GetSeriesServiceProxy } from '@shared/service-proxies/proxy-series';

@Component({
  templateUrl: './series.component.html',
	animations: [appModuleAnimation()]
})
export class SeriesComponent extends PagedListingComponentBase<SeriesDto> {
  active: boolean = false;
  saving: boolean = false;
  code: string;
  group: SeriesDto[];
  listGrid: Array<SeriesDto> = [];
  hold: number = 0;
  hold1: number = 1;
  
  constructor(
    injector: Injector,
    private _gbsService: GetSeriesServiceProxy
  ) {
    super(injector);
  }
  
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    this._gbsService.getSeries(request.skipCount, request.maxResultCount)
      .finally(() => {
        finishedCallback();
      })
      .subscribe((result: PagedResultDtoOfSeriesDto) => {
  ...
  protected delete(entity: SeriesDto): void {
