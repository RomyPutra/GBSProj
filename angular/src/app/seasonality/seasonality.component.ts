...
import { SeasonalityDto, PagedResultDtoOfSeasonalityDto } from '@shared/models/model-seasonality';
import { GetSeasonalityServiceProxy } from '@shared/service-proxies/proxy-seasonality';

@Component({
  templateUrl: './seasonality.component.html',
	animations: [appModuleAnimation()]
})
export class SeasonalityComponent extends PagedListingComponentBase<SeasonalityDto> {
  active: boolean = false;
  saving: boolean = false;
  code: string;
  group: SeasonalityDto[];
  listGrid: Array<SeasonalityDto> = [];
  hold: number = 0;
  hold1: number = 1;
  
  constructor(
    injector: Injector,
    private _gbsService: GetSeasonalityServiceProxy
  ) {
    super(injector);
  }
  
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    this._gbsService.getseasonality(request.skipCount, request.maxResultCount)
      .finally(() => {
        finishedCallback();
      })
      .subscribe((result: PagedResultDtoOfSeasonalityDto) => {
  ...
  protected delete(entity: SeasonalityDto): void {
  ...
  edit(data: any): void {
    if (this.hold === this.hold1) {
      ...
      //   this.saving = false;
      // })
      // .subscribe((result: SeasonalityDto[]) => {
      //   if (result) {
