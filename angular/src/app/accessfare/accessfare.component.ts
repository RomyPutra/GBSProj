...
export class AgentAccessFareComponent extends PagedListingComponentBase<AgentAccessFareDto> {
  active: boolean = false;
  saving: boolean = false;
  code: string;
  group: AgentAccessFareDto[];
  listGrid: Array<AgentAccessFareDto> = [];
  hold: number = 0;
  hold1: number = 1;
  
  constructor(
    injector: Injector,
    private _gbsService: GetAgentAccessFareServiceProxy
  ) {
    super(injector);
  }
  
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    this._gbsService.getAgentAccessFare(request.skipCount, request.maxResultCount)
      .finally(() => {
        finishedCallback();
      })
      .subscribe((result: PagedResultDtoOfAgentAccessFareDto) => {
  ...
  protected delete(entity: AgentAccessFareDto): void {
  ...
  edit(data: any): void {
    if (this.hold === this.hold1) {
      ...
      //   this.saving = false;
      // })
      // .subscribe((result: AgentAccessFareDto[]) => {
      //   if (result) {
