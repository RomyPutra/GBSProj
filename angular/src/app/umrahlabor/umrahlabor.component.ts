...
import { umrahlaborDto, PagedResultDtoOfumrahlaborDto } from '@shared/models/model-umrahlabor';
import { GetumrahlaborServiceProxy } from '@shared/service-proxies/proxy-umrahlabor';

@Component({
  templateUrl: './umrahlabor.component.html',
	animations: [appModuleAnimation()]
})
export class UmrahlaborComponent extends PagedListingComponentBase<umrahlaborDto> {
  active: boolean = false;
  saving: boolean = false;
  code: string;
  group: umrahlaborDto[];
  listGrid: Array<umrahlaborDto> = [];
  hold: number = 0;
  hold1: number = 1;
  
  constructor(
    injector: Injector,
    private _gbsService: GetumrahlaborServiceProxy
  ) {
    super(injector);
  }
  
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    this._gbsService.getumrahlabor(request.skipCount, request.maxResultCount)
      .finally(() => {
        finishedCallback();
      })
      .subscribe((result: PagedResultDtoOfumrahlaborDto) => {
  ...
  protected delete(entity: umrahlaborDto): void {
