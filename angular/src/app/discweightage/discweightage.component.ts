...
import { DiscWeightageDto, PagedResultDtoOfDiscWeightageDto } from '@shared/models/model-discweightage';
import { GetDiscWeightageServiceProxy } from '@shared/service-proxies/proxy-discweightage';

@Component({
  templateUrl: './discweightage.component.html',
	animations: [appModuleAnimation()]
})
export class DiscweightageComponent extends PagedListingComponentBase<DiscWeightageDto> {
  active: boolean = false;
  saving: boolean = false;
  code: string;
  group: DiscWeightageDto[];
  listGrid: Array<DiscWeightageDto> = [];
  hold: number = 0;
  hold1: number = 1;
  
  constructor(
    injector: Injector,
    private _gbsService: GetDiscWeightageServiceProxy
  ) {
    super(injector);
  }
  
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    this._gbsService.getdiscweightage(request.skipCount, request.maxResultCount)
      .finally(() => {
        finishedCallback();
      })
      .subscribe((result: PagedResultDtoOfDiscWeightageDto) => {
  ...
  protected delete(entity: DiscWeightageDto): void {
  ...
  edit(data: any): void {
    if (this.hold === this.hold1) {
      ...
      //   this.saving = false;
      // })
      // .subscribe((result: DiscWeightageDto[]) => {
      //   if (result) {
